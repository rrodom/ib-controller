...
class SwingUtils {
     ...
     *
     * @param window     The window containing the button.
     ...
     * @param text      the label of the button to be found
    ...
    static JButton findButton(Container container, String text) {
        ...
        Utils.logDebugToConsole("SwingUtils.findButton | text=" + text);
        JButton ret = null;
        while (iter.hasNext()) {
            Component component = iter.next();
            if (!(component instanceof JButton)) {
                Utils.logDebugToConsole("SwingUtils.findButton | component.name=" + component.getName());
                Utils.logDebugToConsole("SwingUtils.findButton | component.canonical-name=" + component.getClass().getCanonicalName());
                if (component instanceof JPanel) {
                    Utils.logDebugToConsole("SwingUtils.findButton | JPanel.name=" + ((JPanel) component).getName());
                } else if (component instanceof JLabel) {
                    Utils.logDebugToConsole("SwingUtils.findButton | JLabel.name=" + ((JLabel) component).getName());
                } else if (component instanceof JCheckBox) {
                    Utils.logDebugToConsole("SwingUtils.findButton | JCheckBox.name=" + ((JCheckBox) component).getName());
                }
                continue;
            }
            Utils.logDebugToConsole("SwingUtils.findButton | JButton.text=" + ((JButton) component).getText());
            if (text.equals(((JButton) component).getText())) {
                Utils.logDebugToConsole("SwingUtils.findButton | setting");
                ret = (JButton) component;
                break;
            }
        }
        return ret;
    ...
    static JCheckBox findCheckBox(Container container, String text) {
        ComponentIterator iter = new ComponentIterator(container);
        Utils.logDebugToConsole("SwingUtils.findCheckBox | text=" + text);
        JCheckBox ret = null;
        while (iter.hasNext()) {
            Component component = iter.next();
            ...
            Utils.logDebugToConsole("SwingUtils.findCheckBox | component.text=" + ((JCheckBox) component).getText());
            if (text.equals(((JCheckBox) component).getText())) {
                Utils.logDebugToConsole("SwingUtils.findButton | setting");
                ret = (JCheckBox) component;
                break;
            }
        }
        return ret;
     ...
     *
     * @param container the Container to search in
     * @param ith       specifies which JComboBox to return (the first one is specified by 0,
     *                  the next by 1, etc)
     * @return the required JComboBox if it is found, otherwise null
     */
    static JComboBox<?> findComboBox(Container container, int ith) {
        ComponentIterator iter = new ComponentIterator(container);
        Utils.logDebugToConsole("SwingUtils.findComboBox | ith=" + ith);
        JComboBox<?> ret = null;
        int i = 0;
        while (iter.hasNext()) {
            Component component = iter.next();
            ...
            Utils.logDebugToConsole("SwingUtils.findCheckBox | component.pos=" + i);
            if (i++ == ith) {
                Utils.logDebugToConsole("SwingUtils.findComboBox | setting");
                ret = (JComboBox<?>) component;
                break;
            }
        }
        return ret;
     ...
     *
     * @param container the Container to search in
     * @param text      the label of the Component to be found
     * @return the Component, if it was found;  otherwise null
     */
    static Component findComponent(Container container, String text) {
        ComponentIterator iter = new ComponentIterator(container);
        Utils.logDebugToConsole("SwingUtils.findComponent | text=" + text);
        Component ret = null;
        while (iter.hasNext()) {
            Component component = iter.next();
            Utils.logDebugToConsole("SwingUtils.findComponent | component.text=" + (component.getName()));
            if (text.equals(component.getName())) {
                Utils.logDebugToConsole("SwingUtils.findComponent | setting");
                ret = component;
                break;
            }
        }
    ...
    static JRadioButton findRadioButton(Container container, String text) {
        ComponentIterator iter = new ComponentIterator(container);
        Utils.logDebugToConsole("SwingUtils.findRadioButton | text=" + text);
        while (iter.hasNext()) {
            Component component = iter.next();
    ...
    static JLabel findLabel(Container container, String text) {
        ComponentIterator iter = new ComponentIterator(container);
        Utils.logDebugToConsole("SwingUtils.findLabel | text=" + text);
        while (iter.hasNext()) {
            Component component = iter.next();
            if (component instanceof JLabel && ((JLabel) component).getText() != null && ((JLabel) component).getText().contains(text))
    ...
    static JTextField findTextField(Container container, int ith) {
        ComponentIterator iter = new ComponentIterator(container);
        Utils.logDebugToConsole("SwingUtils.findTextField | ith=" + ith);
        int i = 0;
        while (iter.hasNext()) {
            Component component = iter.next();
    ...
    static JMenuBar findMenuBar(Container container) {
        ComponentIterator iter = new ComponentIterator(container);
        Utils.logDebugToConsole("SwingUtils.findMenuBar");
        while (iter.hasNext()) {
            Component component = iter.next();
    ...
    static JMenuBar findMenuBar(Container container, int ith) {
        ComponentIterator iter = new ComponentIterator(container);
        Utils.logDebugToConsole("SwingUtils.findMenuBar | ith=" + ith);
        int i = 0;
        while (iter.hasNext()) {
            Component component = iter.next();
    ...
    static JMenuItem findMenuItem(MenuElement container, String text) {
        Utils.logDebugToConsole("SwingUtils.findMenuItem | text=" + text);
    ...
    static JMenuItem findMenuItem(Container container, String[] path) {
        ...
        Utils.logDebugToConsole("SwingUtils.findMenuItem");
    ...
    static JMenuItem findMenuItemInAnyMenuBar(Container container, String[] path) {
        if (path.length == 0) return null;
        Utils.logDebugToConsole("SwingUtils.findMenuItemInAnyMenuBar");
        int i = 0;
        while (true) {
    ...
    static JOptionPane findOptionPane(Container container) {
        ComponentIterator iter = new ComponentIterator(container);
        Utils.logDebugToConsole("SwingUtils.findOptionPane");
        while (iter.hasNext()) {
            Component component = iter.next();
    ...
    static JTree findTree(Container container) {
        ComponentIterator iter = new ComponentIterator(container);
        Utils.logDebugToConsole("SwingUtils.findTree");
        while (iter.hasNext()) {
            Component component = iter.next();
    ...
    static Object findChildNode(TreeModel model, Object node, String text) {
        Utils.logDebugToConsole("SwingUtils.findChildNode | text=" + text);
     ...
     *
     * @param window     the window in which to search for the required JButton
     * @param buttonText the label of the required JButton
     * @return true if the JButton is enabled; false if there is no such JButton, or it
     * is disabled
     */
    static boolean isButtonEnabled(final Window window, final String buttonText) {
        Utils.logDebugToConsole("SwingUtils.isButtonEnabled | text=" + buttonText);
        final JButton button = findButton(window, buttonText);
        if (button == null) return false;
    ...
    static boolean isCheckBoxSelected(Window window, String buttonText) {
        Utils.logDebugToConsole("SwingUtils.isCheckBoxSelected | text=" + buttonText);
    ...
    static boolean isRadioButtonSelected(Window window, String buttonText) {
        Utils.logDebugToConsole("SwingUtils.isRadioButtonSelected | text=" + buttonText);
    ...
    static boolean setCheckBoxSelected(Window window, String buttonText, final boolean value) {
        Utils.logDebugToConsole("SwingUtils.setCheckBoxSelected | text=" + buttonText + " | value=" + value);
        final JCheckBox cb = findCheckBox(window, buttonText);
        if (cb == null) return false;
     ...
     *
     * @param window     the window in which to search for the required JRadioButton
     * @param buttonText the label for the required JRadioButton
    ...
    static boolean setRadioButtonSelected(Window window, String buttonText) {
        Utils.logDebugToConsole("SwingUtils.setRadioButtonSelected | text=" + buttonText);
        final JRadioButton rb = findRadioButton(window, buttonText);
        if (rb == null) return false;
    ...
    static boolean setTextField(Window window, int fieldNumber, final String value) {
        ...
        if (tf != null) {
            Utils.logDebugToConsole("SwingUtils.setTextField | setting");
            tf.setText(value);
            return true;
        }
        return false;
    ...
    static boolean titleContains(Window window, String text) {
        ...
        Utils.logToConsole("SwingUtils.titleContains |title=" + title + " |text=" + text);
    ...
    static boolean titleEquals(Window window, String text) {
        String title = getWindowTitle(window);
        Utils.logToConsole("SwingUtils.titleEquals |title=" + title + " |text=" + text);
